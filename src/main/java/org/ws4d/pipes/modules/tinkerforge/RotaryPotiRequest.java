...
public class RotaryPotiRequest extends
		BaseBrickletRequestModule<BrickletRotaryPoti> {

	@Override
	protected BrickletRotaryPoti createDevice(String uid) {
		try {
			return new BrickletRotaryPoti(uid, Activator.getInstance().getConnectionManager()
	...
	protected void doWork() {
		...
		if (device != null) {
			try {
				final Short position = device.getPosition();

				setOutData("out", position.intValue());
			} catch (Exception e) {
				getLogger().log(
						Level.SEVERE,
