...
outPorts = {//
		@OutPort(name = "out", label = "%RH/10 (Relative Humidity)") }//
)
public class HumidityRequest extends
		BaseBrickletRequestModule<BrickletHumidity> {

	@Override
	protected BrickletHumidity createDevice(String uid) {
		try {
			return new BrickletHumidity(uid, Activator.getInstance().getConnectionManager()
	...
	protected void doWork() {
		...
		if (device != null) {
			try {
				setOutData("out", device.getHumidity());
			} catch (Exception e) {
				getLogger().log(
						Level.SEVERE,
						"Can't get humidity from bricklet " + uid + " (" + host
								+ ":" + port + ")", e);
			}
		}
	}
