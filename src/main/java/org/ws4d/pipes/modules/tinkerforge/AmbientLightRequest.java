...
outPorts = {//
@OutPort(name = "out", label = "Illuminance (Lux *10)") }//
)
public class AmbientLightRequest extends
		BaseBrickletRequestModule<BrickletAmbientLight> {

	@Override
	protected BrickletAmbientLight createDevice(String uid) {
		try {
			return new BrickletAmbientLight(uid, Activator.getInstance()
	...
	protected void doWork() {
		...
		if (device != null) {
			try {
				setOutData("out", device.getIlluminance());
			} catch (Exception e) {
				getLogger().log(
						Level.SEVERE,
						"Can't get illuminance from bricklet " + uid + " ("
								+ host + ":" + port + ")", e);
			}
		}
	}
