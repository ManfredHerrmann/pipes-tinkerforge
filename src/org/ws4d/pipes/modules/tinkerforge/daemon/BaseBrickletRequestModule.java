...
public abstract class BaseBrickletRequestModule<T extends Device> extends
	...
	protected void doWork() {
		...
		if (portChanged && (host != null) && (port != null) && (uid != null)) {
			device = (T) ConnectionManager.getInstance().getDevice(host,
					Integer.parseInt(port), uid);
			if (device == null) {
				try {
					device = (T) ConnectionManager.getInstance().addDevice(host,
							Integer.parseInt(port), uid, createDevice(uid));
				} catch (NumberFormatException e) {
					getLogger().log(Level.SEVERE, "invalid brickd port number", e);
				} catch (IOException e) {
					getLogger().log(Level.SEVERE, "error connecting to brickd", e);
				} catch (TimeoutException e) {
					getLogger().log(Level.SEVERE, "error adding bricklet " + uid, e);
				}
			}
		}
	}
